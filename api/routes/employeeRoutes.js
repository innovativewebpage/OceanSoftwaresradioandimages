...
router.post('/create', upload.fields([{
	name: 'omar', maxCount: 1
}, {
	name: 'rizwan', maxCount: 1
}]), async (req, res) => {
	// console.log('ok');
	// console.log('omar==', req.files.omar);
	// console.log('rizwan==', req.files.rizwan);

	//console.log(req.files.file[0].filename);
	//console.log('req.files.file',req.files.file)
	var pro_image = "";
	if (typeof req.files.omar !== 'undefined') {
		pro_image = req.files.omar[0].filename;
	}
	else {
		pro_image = "";
	}
	...
	var emp_resume = ""
	if (typeof req.files.rizwan !== 'undefined') {
		emp_resume = req.files.rizwan[0].filename;
	}
	else {
		emp_resume = "";
	}
